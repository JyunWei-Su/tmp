...
def scan_and_print(root, level_filter, scan_fh, error_fh, svn_fh,
    ...
    for dirpath, dirnames, filenames in os.walk(
        ...
        for dname in list(dirnames):
            ...
            if is_svn_wc(full):
                dirnames.remove(dname)   # prune FIRST so os.walk never descends inside
                if _interrupted:
                    break
                try:
                    st    = full.stat()
                    ...
                    _log_error(error_fh, str(full), exc)
                    continue
                # dir_total_size is safe here: dname was already removed from
                # dirnames, so os.walk will not enter this directory again.
                # The recursive walk happens only inside this SVN dir, which
                # we deliberately skip from the main scan.
                size = dir_total_size(full)
                if size == -1:   # Ctrl-C during size calc
                    break
            ...
            abs_path = str(root_real / full.relative_to(root))
            emit_row(
                fmt_row(size, mt, tag_key, owner, "D", rule_id, abs_path, for_file=False),
                fmt_row(size, mt, tag_key, owner, "D", rule_id, abs_path, for_file=True),
                scan_fh, cur_dir,
            )
            results.append((size, mt, action_lv, tag_key, owner, "D", rule_id, Path(abs_path)))
            counts[action_lv] += 1
        ...
        for fname in filenames:
            ...
            abs_path = str(root_real / full.relative_to(root))
            emit_row(
                fmt_row(size, mt, tag_key, owner, "F", rule_id, abs_path, for_file=False),
                fmt_row(size, mt, tag_key, owner, "F", rule_id, abs_path, for_file=True),
                scan_fh, cur_dir,
            )
            results.append((size, mt, action_lv, tag_key, owner, "F", rule_id, Path(abs_path)))
            counts[action_lv] += 1
            stats.record_scan(rule_id, size)
