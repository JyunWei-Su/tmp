        ...
        for dname in list(dirnames):
            ...
            if is_svn_wc(full):
                ...
                try:
                    ...
                    # Use st_blocks for an instant size estimate -- no recursion,
                    # avoids hanging on large SVN working copies with many .svn files.
                    # st_blocks is in 512-byte units; fall back to 0 on Windows.
                    size  = (getattr(st, "st_blocks", 0) * 512)
                except OSError as exc:
                    _log_error(error_fh, str(full), exc)
                    continue
