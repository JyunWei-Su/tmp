...
def dir_total_size(path):
    # type: (Path) -> int
    """Estimate directory size from the top-level stat only.

    A full recursive walk can take seconds on large directories and blocks
    Ctrl-C response during that time.  Instead we use st_blocks * 512 from
    the directory's own stat, which is instant and good enough for display.
    For a more accurate (but still fast) estimate we do ONE level of scandir
    and sum the immediate children -- this captures the common case where the
    matched directory IS the large artifact (e.g. xcelium.d/).
    We intentionally do NOT recurse so the function returns immediately.
    """
    total = 0
    try:
        # count the dir entry itself
        total += path.stat().st_size
        # one level of children only -- fast, no recursion
...
def open_log(path):
    # type: (str) -> object
    """Open a log file for writing.  If path ends with .gz, use gzip."""
    if path.endswith(".gz"):
