...
Options:
    --output FILE        Write scan results to FILE  (plain text, no color)
    --delete-log FILE    Write delete actions to FILE (plain text, no color)
...
#  Streaming scan
# ===========================================================================
def scan_and_print(root, level_filter, scan_fh):
...
# ===========================================================================
def do_delete(results, dry_run, delete_fh):
    # type: (List[Tuple], bool, object) -> None
    """
    delete_fh : open file handle for the delete log, or None.
    The delete log is written in plain text (no ANSI codes).
    """
...
def build_parser():
    ...
    p.add_argument("--output", metavar="FILE", default=None,
        help="Write scan results to FILE (plain text, no color)")
    p.add_argument("--delete-log", metavar="FILE", default=None,
        dest="delete_log",
        help="Write delete actions to FILE (plain text, no color)")
...
def main():
    ...
    if args.output:
        try:
            scan_fh = open(args.output, "w", encoding="utf-8")
            scan_fh.write("# IC Cleanup -- scan report\n")
            scan_fh.write("# Root  : {}\n".format(root_real))
            scan_fh.write("# Date  : {}\n".format(
            ...
            print("ERROR: cannot open scan log: {}".format(exc), file=sys.stderr)
            sys.exit(1)
    ...
    if args.delete_log:
        try:
            delete_fh = open(args.delete_log, "w", encoding="utf-8")
        except OSError as exc:
            print("ERROR: cannot open delete log: {}".format(exc), file=sys.stderr)
            if scan_fh:
                scan_fh.close()
            sys.exit(1)

    try:
        results = scan_and_print(root, level_filter, scan_fh)
    finally:
        if scan_fh:
            scan_fh.close()
            print("\nScan report written to: {}".format(
    ...
    print()
    try:
        do_delete(results, dry_run=not args.delete, delete_fh=delete_fh)
    finally:
        if delete_fh:
            delete_fh.close()
            print("\nDelete log written to: {}".format(
                _c(str(Path(args.delete_log).resolve()), "cyan")))
